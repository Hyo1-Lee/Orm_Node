...
- DTO (Data Transfer Object) Model: 사용 목적은 단일 호출로 보다더 많은 데이터를 한번에 전달하기위해서 사용된다.

## ORM 기반 개발 방식 2가지
1. Code First 방식 (model first)
	- 모델 코드를 먼저 만들고 만든 모델을 기반으로 해당 Database에 물리적 테이블을 생성
2. DB First 방식
	- DB에 물리적 테이블을 먼저 만들거나 이미 만들어진 테이블 기반으로 모델을 자동으로 생성해서 모델 기반 프로그래밍 하는 기법
